...
2. Tutaj każdą literkę wyodrębnisz i wykorzystasz oddzielnie. Do zmiennej q zapisz tekst "THE EYES". Wyświetl napis
zbudowany z liter zmiennej q w kolejności: 0,1,2,5,3,7,4,6. Wyświetlając literki skorzystaj z print lub dodaj do siebie literki budując napis.
...
5. Zostajesz zatrudniony w firmie, która wykonuje analizę oglądalności poszczególnych programów TV. Na bardzo początkowym
etapie, Twój program musi przeczytać dane z pliku tekstowego z zapisanym harmonogramem poszczególnych programów.
Plik zawiera linie zbudowane tak, że tytuł programu znajduje się w cudzysłowie, a kończy się napisem o:, po którym następuje godzina, np tak:
...
q="THE MORSE CODE"
print(q[1:3],q[6],q[2],q[9:12],q[4],q[2],q[12],q[11],q[0],q[7])

line = "'Program \"Kropka nad i\" nadamy o: 22:00'"
tmp = line[line.find(":")+1:]
time = tmp[:tmp.find("'")]
tmp2 = line[line.find('"')+1:]
title = tmp2[:tmp2.find('"')]
#print(line)
